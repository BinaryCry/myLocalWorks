...
<html>
	...
	<?php
		...
		*/
		...
		$a="0"; $b=0; $c=""; // true false false; НОЛЬ С НУЛЕМ ПРИВЕДУТСЯ В ЧИСЛО (СТРОКА РАВНА ЧИСЛУ, С КОТОРОГО ОНА НАЧИНАЕТСЯ (ЧИСЛА ВНУТРИ СТРОКИ РОЛИ НЕ ИГРАЮТ); ТУТ БУДЕТ 0)
		echo $a==$b ? 'Y' : 'N', $b==$c ? 'Y' : 'N', $a==$c ? 'Y' : 'N'; // $b == $c true потому, что строка без чисел (символьное число приведется в обычное число) будет == нулю ("5text" == 5 true)
    ...
    <!--    Виды комментариев в сокращенном echo ( = )-->
    <?//=PHP_VERSION.'<br>'?>
    <?#=date('Y')?>
